...
- **Fixed**
  - Fixed the **Data ingestion** page in the Azure Data Explorer dashboard ignoring the **Cloud provider** filter. The tiles now derive ingested row counts, scopes, and months from extent tags in a regular query instead of a `.show cluster extents` management command, which Azure Data Explorer cannot combine with dashboard parameters. Non-Azure scopes, such as AWS accounts, also show an identifier instead of a blank label.
  - Fixed the **Anomaly management** page appearing broken when a provider's data stops arriving. Every tile on the page uses a rolling window relative to today, so a stale source rendered as an empty page with no explanation. A **Data freshness by provider** tile now reports the latest charge date and how many days behind each provider is.
